...
export default class CampaignHeaderV2 extends Component {
  ...
  render() {
    ...
    const isDisplayMinTarget = !get(campaign, 'header.toggleMeta').includes('MINIMUM_TARGET');
    const isDisplayMaxTarget = !get(campaign, 'header.toggleMeta').includes('MAXIMUM_TARGET');
    return (
      <>
        {!isMobile
          ? (
