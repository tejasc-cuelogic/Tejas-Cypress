...
import { Container, Header, Table, Card, Visibility, Item, Modal, Button } from 'semantic-ui-react';
                  ...
                  <Table unstackable striped className="user-list email-list">
                    <Table.Header>
                      <Table.Row>
                        ...
                        <Table.HeaderCell>Mergevars</Table.HeaderCell>
                      ...
                      {!userEmals || (userEmals && userEmals.length === 0) ? (
                        ...
                        userEmals && userEmals.map((resp, idx) => (
                            <Table.Row>
                              ...
                              <Table.Cell collapsing>{resp.subject}</Table.Cell>
                              <Table.Cell>
                                <DateTimeFormat isCSTFormat datetime={DataFormatter.getDateAsPerTimeZone(resp.requestDate, true, false, false)} />
                              </Table.Cell>
                              <Table.Cell>
                                <Item>
                                ...
                                <Item>
                                  {(resp.attachments === null) || (resp.attachments && resp.attachments.length <= 0) ? 'NA'
                                    : resp.attachments.map(attach => (
                                      <Item.Content>
                                        <b>Content: </b>{attach.content ? attach.content : 'N/A'}
                                      </Item.Content>
                                    ))
                                  }
                                </Item>
                              </Table.Cell>
                              <Table.Cell collapsing><Link onClick={e => this.handleModel(e, idx)} to="/">Show Email Content</Link></Table.Cell>
                            </Table.Row>
                        ))
                      )
                      }
