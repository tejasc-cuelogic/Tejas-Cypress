...
if [ "$isBucketExist" != "" ]; then
	...
	sed -i "s/Bucket-Name/${bucketName}/" s3PublicReadPolicy.json && echo "policy created"
	aws s3api put-bucket-policy --bucket ${bucketName} --policy file://s3PublicReadPolicy.json && echo "Policy attached."
