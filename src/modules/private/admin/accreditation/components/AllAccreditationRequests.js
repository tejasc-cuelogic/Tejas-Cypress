              ...
              accreditations.length === 0 ? (
                ...
                : accreditations.map(accreditation => (
                  <Table.Row key={accreditation.id}>
                    <Table.Cell>
                      <Link onClick={() => sessionStorage.setItem('userDetailsRefUrl', match.url)} to={`/app/users/${accreditation.userId}/profile-data`}><p><b>{`${accreditation.firstName} ${accreditation.lastName}`}</b></p></Link>
                    </Table.Cell>
                    <Table.Cell>
