...
settingEnv()
{
	...
	REACT_APP_API_URL=$(cat Env.txt | awk -F '/\/ns-client\/'$environment'\/api\/url/ { print $3 }')
    ...
    REACT_APP_PUBLIC_API=$(cat Env.txt | awk -F '/\/ns-client\/'$environment'\/api\/public\/url/ { print $3 }')
    if [ "$environment" = "predev" ]; then
