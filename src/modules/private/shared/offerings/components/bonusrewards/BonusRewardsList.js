            ...
            || (this.props.isEarlyBird && reward.earlyBirdQuantity > 0)) {
              return (
                <div className="reward-wrap">
                  <Header as="h5">
                    {reward.title}
                    {reward.expirationDate && <small className="note">  - Exp Date: {DataFormatter.getDateAsPerTimeZone(reward.expirationDate, false, false, false, 'MMM D, YYYY', 'CST', true)}</small>}
                    {!isReadOnly
