      ...
      <Grid stackable doubling>
        ...
        && InsightArticles.map(article => (
          <Grid.Column mobile={16} tablet={8} computer={4}>
            <Card className="campaign insights" fluid as={Link} to={`/insights/${article.slug}`}>
              <Image64
                centered
              ...
              <Card.Content>
                <div className="tags">
                  {article.category}
                  <span className="pull-right read-duration">{article.minuteRead ? `${article.minuteRead} Min read` : ''}</span>
                </div>
                <Card.Description>
                  {article.title}
                </Card.Description>
