...
import { isEmpty, omit, find, forEach } from 'lodash';
...
class EntityAccountStore {
  ...
  addToJson(mainObj, responseObj, array) {
    const resObj = responseObj;
    forEach(array, (val) => {
      resObj[val] = mainObj[val].value;
    });
    return responseObj;
  }
  ...
  get accountAttributes() {
    let responseObj = {
      netAssets: this.FIN_INFO_FRM.netAssets.value,
      cfInvestment: {
        dateOfInvestment: '23',
        amount: this.FIN_INFO_FRM.cfInvestment.value,
      },
    };

    const array1 = ['name', 'taxId', 'isTrust', 'trustDate'];
    responseObj = this.addToJson(this.GEN_INFO_FRM.fields, responseObj, array1);


    const array2 = ['street', 'city', 'state', 'zipCode'];
    responseObj.entity = {};
    responseObj.entity.address = {};
    responseObj = this.addToJson(this.GEN_INFO_FRM.fields, responseObj, array2);

    const array3 = ['legalFirstName', 'legalLastName', 'title'];
    responseObj.legalInfo = {};
    responseObj = this.addToJson(mainObj, responseObj, array3);

    const array4 = ['fileId', 'fileName'];
    responseObj.entity.legalInfo = {};
    responseObj.entity.legalInfo.legalDocUrl = {};
    responseObj = this.addToJson(this.PERSONAL_INFO_FRM.fields, responseObj, array4);

    // same keys so used array4 only
    responseObj.entity.legalDocs = {};
    responseObj.entity.legalDocs.formationDoc = {};
    responseObj = this.addToJson(this.FORM_DOCS_FRM.fields, responseObj, array4);

    responseObj.entity.legalDocs.operatingAgreementDoc = {};
    responseObj = this.addToJson(this.FORM_DOCS_FRM.fields, responseObj, array4);

    responseObj.entity.legalDocs.einVerificationDoc = {};
    responseObj = this.addToJson(this.FORM_DOCS_FRM.fields, responseObj, array4);
