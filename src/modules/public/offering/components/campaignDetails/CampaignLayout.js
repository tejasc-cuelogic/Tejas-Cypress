...
class CampaignLayout extends Component {
  ...
  render() {
    const { campaign, campaignStatus, dataRoomDocs, isPostedNewComment } = this.props.campaignStore;
    ...
    return (
      <div className="campaign-content-wrapper v-2">
        ...
        <>
          ...
          {((postedComments.length > 1) || (isPostedNewComment && postedComments.length > 1))
            && (
