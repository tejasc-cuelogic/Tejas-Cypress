...
const CollectionItem = ({ isMobile, collection }) => (
	<>
		<section key={collection.id} className={`${responsiveVars.uptoTablet ? 'pt-50 pb-50' : 'pt-100 pb-100'} bg-offwhite`}>
			<Container>
				<Grid>
					<Grid.Column widescreen={7} computer={7} tablet={16} mobile={16}>
						<div>
						<NSImage path={collection.logo} />
						<p className="mb-30 mt-30">{collection.description}</p>
						{collection.link && !isMobile && !isTablet
							&& (
							<a href={collection.link} target="_blank" rel="noopener noreferrer"><Button inverted color="green" className="mb-30">Learn More</Button></a>
							)
						}
						</div>
					</Grid.Column>
					<Grid.Column widescreen={8} computer={8} tablet={16} mobile={16} floated={!isMobile ? 'right' : ''}>
						<NSImage path={collection.image} fluid />
						{collection.link && (isMobile || isTablet)
						&& (
							<a href={collection.link} target="_blank" rel="noopener noreferrer"><Button inverted fluid={isMobile} color="green" className="mt-30">Learn More</Button></a>
						)
						}
					</Grid.Column>
				</Grid>
...
export default class Collections extends Component {
 render(){
    return(
			this.collectionsList.map(collection => (
