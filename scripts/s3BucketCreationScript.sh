...
if [ "$isBucketExist" != "" ]; then
	...
	sed -i "s/Bucket-Name/${bucketName}/" S3PublicReadAccessPolicy.json && echo "policy created"
	aws s3api put-bucket-policy --bucket ${bucketName} --policy file://S3PublicReadAccessPolicy.json && echo "Policy attached."
