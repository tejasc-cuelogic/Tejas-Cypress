                  ...
                  <>
                    ...
                    <div className="bg-offwhite">
                      <dl className="dl-horizontal">
                        <dt>Status :</dt>
                        <b><dd className={`${(get(accreditationData[account.name], 'status') === 'CONFIRMED' && get(accreditationData[account.name], 'expiration') && (DataFormatter.diffDays(DataFormatter.formatedDate(get(accreditationData[account.name], 'expiration')), false, true) < 0)) ? 'negative' : get(accreditationData[account.name], 'status') === 'REQUESTED' ? 'warning' : get(accreditationData[account.name], 'status') === 'CONFIRMED' ? 'positive' : get(accreditationData[account.name], 'status') === 'INVALID' ? 'negative' : ''}-text`}>{get(accreditationData[account.name], 'status') === 'CONFIRMED' && get(accreditationData[account.name], 'expiration') && (DataFormatter.diffDays(DataFormatter.formatedDate(get(accreditationData[account.name], 'expiration')), false, true) < 0) ? 'Expired' : get(accreditationData[account.name], 'status') ? ACCREDITATION_STATUS_LABEL[get(accreditationData[account.name], 'status')] : 'N/A'}</dd></b>
                      </dl>
