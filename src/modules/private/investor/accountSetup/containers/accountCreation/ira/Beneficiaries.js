...
export default class Beneficiaries extends Component {
  constructor(props) {
    super(props);
    this.props.iraAccountStore.populateNestedSteps();
  }

  handleStepChange = (e, result, formName) => {
    e.preventDefault();
    const { formChange, handleNestedStepChange, setStepToBeRendered, stepToBeRendered } = this.props.iraAccountStore;
    formChange(e, result, formName);
    if (result.value === 'no') {
      setStepToBeRendered(stepToBeRendered + 1);
    } else {
      handleNestedStepChange();
    }
  }
  ...
  render() {
    ...
    const nestedSteps = [
      ...
      {
        component: <Beneficiary isMobile={responsiveVars.isMobile} handleStepChange={this.handleStepChange} />,
        step: 2,
      },
    ];
    const currentStep = currentNestedStep < nestedSteps.length ? nestedSteps.find(s => s.step === currentNestedStep) : nestedSteps.slice(-1)[0];
