            ...
            <Grid.Column key={snakeCase(row.title)}>
              <Card.Content>
                <Statistic size="mini" horizontal={isMobile} className={row.status}>
                  ...
                  <Statistic.Value>{props.loading ? 'loading...' : props.stage === 'DEFAULTED' && (row.title === 'Principal Remaining' || row.title === 'Payments Remaining') ? '$0.00' : showValue(row)}</Statistic.Value>
                </Statistic>
              </Card.Content>
              {row.title === 'Total Balance'
