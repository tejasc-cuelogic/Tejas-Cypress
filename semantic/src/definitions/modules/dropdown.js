        ...
        module = {
              ...
              $item
                .each(function () {
                  ...
                  if (settings.match == 'both' || settings.match == 'text') {
                    ...
                    if (settings.fullTextSearch === 'exact' && module.exactSearch(searchTerm, text)) {
                      results.push(this);
                      return true;
                    }
                    if (settings.fullTextSearch === true && module.fuzzySearch(searchTerm, text)) {
                      results.push(this);
                      return true;
                    }
                  }
                  if (settings.match == 'both' || settings.match == 'value') {
                    ...
                    if (settings.fullTextSearch === 'exact' && module.exactSearch(searchTerm, value)) {
                      results.push(this);
                      return true;
                    }
                    if (settings.fullTextSearch === true && module.fuzzySearch(searchTerm, value)) {
                      results.push(this);
                      return true;
                    }
                  }
          ...
          event: {
            ...
            // label selection should occur even when element has no focus
              keydown(event) {
                ...
                if (isShortcutKey) {
                  ...
                  const caretAtStart = (isFocusedOnSearch && module.get.caretPosition() === 0);
                  let $nextLabel;
            ...
            keydown(event) {
              ...
              if (isShortcutKey) {
                ...
                const delimiterPressed = (pressedKey == keys.delimiter && settings.allowAdditions && module.is.multiple());
                const isAdditionWithoutMenu = (settings.allowAdditions && settings.hideAdditions && (pressedKey == keys.enter || delimiterPressed) && selectedIsSelectable);
                let $nextItem;
                let isSubMenuItem;
                let newIndex
              ;
          ...
          get: {
            ...
            value() {
              ...
              const isEmptyMultiselect = ($.isArray(value) && value.length === 1 && value[0] === '')
            ;
          ...
          restore: {
            ...
            defaultText() {
              ...
              const { placeholderText } = module.get;
          ...
          add: {
            ...
            optionValue(value) {
              const
                escapedValue = module.escape.value(value);
              const $option = $input.find(`option[value="${module.escape.string(escapedValue)}"]`);
            ...
            userSuggestion(value) {
              ...
              const alreadyHasValue = $existingItem && $existingItem.not(selector.addition).length;
          ...
          remove: {
            ...
            label(value, shouldAnimate) {
              ...
              const $removedLabel = $labels.filter(`[data-${metadata.value}="${module.escape.string(value)}"]`);
