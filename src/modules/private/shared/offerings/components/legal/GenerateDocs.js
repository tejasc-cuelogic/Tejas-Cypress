...
export default class GenerateDocs extends Component {
  ...
  generateDocList = (securities, equityClass) => {
    ...
    } else if (securities === CAMPAIGN_KEYTERMS_SECURITIES_ENUM.PREFERRED_EQUITY_506C || (securities === CAMPAIGN_KEYTERMS_SECURITIES_ENUM.EQUITY && ['PREFERRED', 'CLASS_A_SHARES', 'CLASS_B_SHARES', 'PARALLEL_CLASS_SHARES'].includes(equityClass))) {
      documentLists = [...documentLists, 'purchaseAgreement', 'proxyAgreement'];
    } else if (securities === CAMPAIGN_KEYTERMS_SECURITIES_ENUM.REAL_ESTATE || (securities === CAMPAIGN_KEYTERMS_SECURITIES_ENUM.EQUITY && ['LLC_MEMBERSHIP_UNITS'].includes(equityClass))) {
  ...
  render() {
    ...
    const documentLists = this.generateDocList(securities, equityClass);
    return (
